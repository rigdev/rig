...
func (p *Plugin) Run(_ context.Context, req pipeline.CapsuleRequest, _ hclog.Logger) error {
	config, err := mod.ParseTemplatedConfig[Config](p.configBytes, req,
		mod.CapsuleStep[Config],
		func(c Config, req pipeline.CapsuleRequest) (string, any, error) {
			name := c.Name
			if name == "" {
				name = req.Capsule().Name
			}
			currentObject, err := mod.GetNew(c.Group, c.Kind, name, req)
			if err != nil {
				return "", nil, err
			}
	...
	currentObject, err := mod.GetNew(config.Group, config.Kind, name, req)

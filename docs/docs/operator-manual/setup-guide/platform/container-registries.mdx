...
For Container Registries registered in the Rig Platform, [image-pull-secrets](https://kubernetes.io/docs/tasks/configure-pod-container/pull-image-private-registry/#create-a-secret-by-providing-credentials-on-the-command-line) will automatically be generated for the Capsules that uses images from these registries if configured so for the cluster.

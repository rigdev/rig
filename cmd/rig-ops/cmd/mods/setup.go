...
func Setup(parent *cobra.Command, s *cli.SetupContext) {
	modsCmd := &cobra.Command{
		Use:               "mods",
		Aliases:           []string{"plugins"}, // for backwards compatibility
	...
	check := &cobra.Command{
		Use:   "check",
		Short: "Check which mods will be run on which capsules",
	...
	modsCmd.AddCommand(check)

	listSteps := &cobra.Command{
		Use:   "list-steps",
		Short: "Lists the mod steps currently configured in the operator",
		RunE:  cli.CtxWrap(cmd.listSteps),
	}
	//nolint:lll
	listSteps.Flags().BoolVar(&showConfig, "show-config", false, "If set, will also display the YAML configuration for each mod.")
	modsCmd.AddCommand(listSteps)
...
Besides giving a complete list of mod configurations, is possible to edit the mod configuration
...
The dry run will be executed with the resulting list of mod.`,
	...
	//nolint:lll
	dryRun.Flags().StringVar(&modConfig, "config", "", "If given, will read the config file at the path and use that as the pipeline config. If empty, will use the mod config of the running operator.")
	//nolint:lll
	...
	//nolint:lll
	dryRun.Flags().BoolVar(&dry, "dry", false, "If given, will only show the list of mods used for the dry-run")
	//nolint:lll
	...
	modsCmd.AddCommand(dryRun)

	list := &cobra.Command{
		Use:   "list",
		Short: "Lists the set of mods available in the operator",
		RunE:  cli.CtxWrap(cmd.list),
	}
	modsCmd.AddCommand(list)

	parent.AddCommand(modsCmd)
}

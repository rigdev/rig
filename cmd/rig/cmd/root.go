...
func Run() error {
	...
	rootCmd.PersistentFlags().StringVarP(&flags.Flags.Environment,
		"environment", "E", flags.Flags.Environment, "select which environment to use. Can also be set with environment variable `RIG_ENVIRONMENT`")
	rootCmd.PersistentFlags().StringVarP(&flags.Flags.Project,
		"project", "P", flags.Flags.Project, "select which project to use. Can also be set with environment variable `RIG_PROJECT`")
	...
	rootCmd.PersistentFlags().StringVarP(&flags.Flags.Host,
		"host", "H", flags.Flags.Host, "select which host to access the Rig Platform at. Should be of the form `http[s]://hostname:port/`. Can also be set with environment variable `RIG_HOST`")

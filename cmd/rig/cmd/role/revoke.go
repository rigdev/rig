...
func (c *Cmd) revoke(ctx context.Context, cmd *cobra.Command, args []string) error {
	var roleID string
	var groupID string
	// Get role ID
	if len(args) == 0 {
	...
	if _, err := c.Rig.Role().Revoke(ctx, connect.NewRequest(&role.RevokeRequest{
		RoleId: roleID,
	...
	cmd.Println("Revoked role:", roleID, "from group:", groupID)

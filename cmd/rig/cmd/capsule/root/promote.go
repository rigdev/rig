...
func (c *Cmd) promote(ctx context.Context, cmd *cobra.Command, args []string) error {
	...
	changes, err := field.Compare(toSpec, fromSpec)
	if err != nil {
		return err
	}
	...
	if !force {
		...
		changes, err = c.promptChanges(ctx, capsuleID, toEnvID, projectID, fromRollout.Spec, toRollout.Spec, changes)
		if err != nil {
			return err
		}
	}
	...
	deployResp, err := c.Rig.Capsule().Deploy(ctx, connect.NewRequest(&capsule.DeployRequest{
		CapsuleId:     capsuleID,
		ProjectId:     projectID,
	...
	if dryRun {
		for name, resource := range deployResp.Msg.GetResourceYaml() {
			cmd.Println(name)
...
func (c *Cmd) promptChanges(
	...
	from, to *platformv1.CapsuleSpec,
	changes *field.Diff,
) (*field.Diff, error) {
	...
	app.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		...
		case tcell.KeyBackspace2, tcell.KeyBackspace:
			// changes = append(changes[:list.GetCurrentItem()], changes[list.GetCurrentItem()+1:]...)
			...
			from, err = v1.RolloutConfigToCapsuleSpec(resp.Msg.GetRolloutConfig())
			if err != nil {
				errChan <- err
			}

			changes, err = field.Compare(from, to)
			if err != nil {
				errChan <- err
			}
